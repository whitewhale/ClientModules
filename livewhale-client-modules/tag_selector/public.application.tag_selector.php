...
$_LW->REGISTERED_APPS['tag_selector']=[
	'title'=>'Tag Selector',
	'handlers'=>['onLoad','onFormatPublicSubmission']
...
if (!$global_tags=$_LW->getVariable($key)) { // if value not cached
	...
	if ($res=$_LW->dbo->query('select', 'id, title', 'livewhale_tags', 'gid IS NULL AND is_starred IS NOT NULL', 'title ASC')->run()) {
		while ($res2=$res->next()) {
			$tags[]=['title'=>$_LW->setFormatClean($res2['title']), 'id'=>$res2['id']];
		};
	};
	if (!empty($tags)) { // if tags found
		$arr=[0=>[], 1=>[]]; // distribute tags into 2 columns
		...
		$ul1=$xml1->insert($xml1->ul('', ['class' => 'first']));
		$ul2=$xml2->insert($xml2->ul());
		foreach($arr[0] as $key=>$val) { // assign categories to table
		$ul1->insert($xml1->li('<label><input type="checkbox" name="global_tags[]" value="'.str_replace('"', '', $val['title']).'"/> '.$val['title'].'</label>', ['id'=>'tag'.$val['id']]));
			if (isset($arr[1][$key])) {
				$ul2->insert($xml2->li('<label><input type="checkbox" name="global_tags[]" value="'.str_replace('"', '', $arr[1][$key]['title']).'"/> '.$_LW->setFormatClean($arr[1][$key]['title']).'</label>', ['id'=>'tag'.$arr[1][$key]['id']]));
			};
		};
