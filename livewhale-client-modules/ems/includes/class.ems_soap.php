		...
		if ($bookings=$res2->query('/Bookings/Data')) { // fetch and format results
			foreach($bookings as $booking) {
				if ($booking->hasChildNodes()) {
					...
					if (!empty($item['title']) && !empty($item['group_title']) && (empty($groups) || (is_array($groups) && in_array($item['group_title'], $groups)) || (!empty($_LW->REGISTERED_APPS['ems']['custom']['groups_map']) && is_array($_LW->REGISTERED_APPS['ems']['custom']['groups_map']) && in_array($item['group_title'], $_LW->REGISTERED_APPS['ems']['custom']['groups_map']))) && (empty($buildings) || in_array($item['building_id'], $buildings)) && (empty($statuses) || in_array($item['status_id'], $statuses)) && (empty($event_types) || in_array($item['event_type_id'], $event_types)) && (empty($group_types) || in_array($item['group_type_id'], $group_types))) { // if each result is valid
